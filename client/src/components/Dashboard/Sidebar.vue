...
const menuGroups = ref([
  {
    menuItems: [
      {
        icon: `<svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {
        icon: `<svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                <path d="M9 0C4.03125 0 0 4.03125 0 9C0 13.9688 4.03125 18 9 18C13.9688 18 18 13.9688 18 9C18 4.03125 13.9688 0 9 0ZM9 16.5C4.8675 16.5 1.5 13.1325 1.5 9C1.5 4.8675 4.8675 1.5 9 1.5C13.1325 1.5 16.5 4.8675 16.5 9C16.5 13.1325 13.1325 16.5 9 16.5Z" />
                <path d="M9 4.5C7.785 4.5 6.75 5.535 6.75 6.75C6.75 7.965 7.785 9 9 9C10.215 9 11.25 7.965 11.25 6.75C11.25 5.535 10.215 4.5 9 4.5ZM9 7.5C8.585 7.5 8.25 7.165 8.25 6.75C8.25 6.335 8.585 6 9 6C9.415 6 9.75 6.335 9.75 6.75C9.75 7.165 9.415 7.5 9 7.5Z" />
                <path d="M13.5 13.5C13.5 11.5425 11.9575 10 10 10H8C6.0425 10 4.5 11.5425 4.5 13.5C4.5 13.815 4.735 14.0625 5.0625 14.0625H12.9375C13.265 14.0625 13.5 13.815 13.5 13.5ZM6.1875 13.5C6.365 12.8175 7.115 12.375 8 12.375H10C10.885 12.375 11.635 12.8175 11.8125 13.5H6.1875Z" />
              </svg>`,
        label: 'Clients',
      ...
      {
        icon: `<svg class="fill-current" width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {
        icon: `<svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
                <path d="M5.25 3H12.75C13.5784 3 14.25 3.67157 14.25 4.5V13.5C14.25 14.3284 13.5784 15 12.75 15H5.25C4.42157 15 3.75 14.3284 3.75 13.5V4.5C3.75 3.67157 4.42157 3 5.25 3ZM5.25 1.5C3.59315 1.5 2.25 2.84315 2.25 4.5V13.5C2.25 15.1569 3.59315 16.5 5.25 16.5H12.75C14.4069 16.5 15.75 15.1569 15.75 13.5V4.5C15.75 2.84315 14.4069 1.5 12.75 1.5H5.25Z" />
  ...
  },
  {
    menuItems: [
      {
        icon: `<svg class="fill-current" width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg">
...
onMounted(() => {
  ...
  for (let i = 0; i < menuGroups.value.length; i++) {
    for (let j = 0; j < menuGroups.value[i].menuItems.length; j++) {
      if (!hasRole(menuGroups.value[i].menuItems[j].roles)) {
        menuGroups.value[i].menuItems.splice(j, 1);
        j--;
      }
    }
  }
...
<template>
  <aside
    ...
    <div class="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
      <router-link to="/">
        <img src="../../assets/img/POKESHOP_LOGO.png" alt="Logo" class="" />
      </router-link>
    ...
    <div class="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
      <nav class="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
        <template v-for="menuGroup in menuGroups" :key="menuGroup.name">
          <div>
            <h3 class="mb-4 ml-4 text-sm font-medium text-bodydark2" v-if="menuGroup.name">{{ menuGroup.name }}</h3>
