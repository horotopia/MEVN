...
const loadOrders = async () => {
  ...
  const productUrl = `${__VITE_API_URL__}/api/product/countProductSellInMonth`;
  const userUrl = `${__VITE_API_URL__}/api/users/countUsersByMonth`;
  const orderUrl = `${__VITE_API_URL__}/api/orders/calculateAverageOrderAmount`;
  const sessionUrl = `${__VITE_API_URL__}/api/auth/countSessionByMonth`;

  const [productData, userData, orderData, sessionData] = await Promise.all([
    cardFetch(productUrl),
    cardFetch(userUrl),
    cardFetch(orderUrl),
    cardFetch(sessionUrl),
  ]);

  if (productData) productData.value = productData;
  if (userData) userData.value = userData;
  if (orderData) orderData.value = orderData;
  if (sessionData) sessionData.value = sessionData;
